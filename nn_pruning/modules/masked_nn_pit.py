...
class MaskModule(nn.Module):
    ...
    def mask(
        ...
        if method == "topK":
            ...
            conv = torch.nn.Conv2d(1, 1,(args.block_rows, args.block_cols), (args.block_rows, args.block_cols), bias=False).cuda()
            conv.weight.data[:] = 1
            conv.eval()
            mask = MagnitudeBinarizer.apply(conv(weight.unsqueeze(0).unsqueeze(0)).squeeze(), threshold)
...
class MaskedLinear(ReplacementModule):
    def __init__(
        ...
        if args.submethod.startswith("1d"):
            ...
            self.sparse = BlockwiseSparseLinear(linear_module.cuda()) 
        ...
        if row_additive_mask is not None:
            row_additive_mask = row_additive_mask.to(linear_module.device)
        self.row_additive_mask = row_additive_mask

        if col_additive_mask is not None:
            col_additive_mask = col_additive_mask.to(linear_module.device)
